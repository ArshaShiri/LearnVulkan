  ...
  : device_{device}
{
    ...
    if (vkCreateDescriptorPool(device_.device(), &descriptorPoolInfo, nullptr, &descriptorPool_) != VK_SUCCESS)
    {
...
void DescriptorPool::freeDescriptors(std::vector<VkDescriptorSet> &descriptors) const
{
    vkFreeDescriptorSets(
      device_.device(), descriptorPool_, static_cast<uint32_t>(descriptors.size()), descriptors.data());
}
...
{
    assert(setLayout_.bindings.count(binding) == 1 && "Layout does not contain specified binding");

    auto &bindingDescription = setLayout_.bindings[binding];
